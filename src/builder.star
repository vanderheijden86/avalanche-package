...
def init(plan, node_cfg, subnet_genesis_json):
    ...
    image = ImageBuildSpec(
        ...
        # MANDATORY
        build_context_dir="../go_img_with_dlv"
    )
    ...
    plan.add_service(
        ...
        config=ServiceConfig(
            ...
            ports = {
                "dlv": PortSpec(number = 2345, transport_protocol = "TCP", wait = None),
            }
        ),
    )
...
def create_subnet(plan, uri, num_nodes, is_elastic, vmId, chainName):
    plan.exec(
        ...
        recipe=ExecRecipe(
            command=["/bin/sh", "-c",
                     "cd /tmp/wallet && go run main.go {0} {1} {2} {3} {4}".format(uri, vmId, chainName, num_nodes,
                                                                                   is_elastic)]
        )
    )
    ...
    validatorIds = []
    for index in range(0, num_nodes):
        validatorIds.append(utils.read_file_from_service(plan, BUILDER_SERVICE_NAME,
                                                         "/tmp/subnet/node-{0}/validator_id.txt".format(index)))
